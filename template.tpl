...
if (eecAction) {
	...
	if (eecItems) {
        ...
        if (eecAction === 'impressions') {
            ecom = {ecommerce: {impressions: items}};
		}
        if (eecAction === 'click') {
            const item_list_name = items[0].list ? items[0].list : undefined;
            ecom = {ecommerce: {click: {actionField: {list: item_list_name}, products: items}}};
		}
        if (eecAction === 'detail') {
            const item_list_name = items[0].list ? items[0].list : undefined;
            ecom = {ecommerce: {detail: {actionField: {list: item_list_name}, products: items}}};
		}
        if (eecAction === 'add') {
            const item_list_name = items[0].list ? items[0].list : undefined;
            ecom = {ecommerce: {add: {actionField: {list: item_list_name}, products: items}}};
		}
        if (eecAction === 'checkout') {
            ecom = {ecommerce: {checkout: {actionField: {step: checkoutStep, option: checkoutOption}, products: items}}};
		}
        if (eecAction === 'checkout_option') {
            ecom = {ecommerce: {checkout: {actionField: {step: checkoutStep, option: checkoutOption}}}};
		}
        if (eecAction === 'remove') {
            ecom = {ecommerce: {remove: {products: items}}};
		}
        if (eecAction === 'purchase') {
            const transaction_id = ecom.transaction_id, 
                  ...
                  coupon = ecom.coupon;
            ecom = {ecommerce: {purchase: {actionField: {id: transaction_id, affiliation: affiliation, revenue: value, tax: tax, shipping: shipping, coupon: coupon}, products: items}}};
		}
        if (eecAction === 'refund') {
            const transaction_id = ecom.transaction_id;
            ecom = {ecommerce: {refund: {actionField: {id: transaction_id}, products: items}}};
		}
	}
